#!/usr/bin/env python3
"""
Rigorous evaluation: accuracy, F1 (macro/weighted/per-class), recall, precision, confusion matrix,
and quality MAE / R².
...
def eval_bilstm(
    ...
    if text_dim > 0:
        ...
        if not enc_path.is_file():
            raise RuntimeError(
                f"Checkpoint has text_dim={text_dim} but missing encoder: {enc_path}"
            )
        enc = TextCoachingEncoder.load(enc_path)
        if enc.dim != text_dim:
            raise RuntimeError(
                f"Encoder dim {enc.dim} != checkpoint text_dim {text_dim} ({enc_path})"
            )
        ds.set_text_features(enc.transform(ds._text_raw))
...
def main() -> int:
    ...
    for model_type, ckpt_path in args.checkpoint:
        ...
        cm = detailed_classification_metrics(yt, yp, classes)
    ...
    for r in results:
        ...
        print(f"  F1 (weighted):   {c['f1_weighted']:.4f}")
        ...
        for name, v in sorted(c["recall_per_class"].items()):
            print(f"    {name}: {v:.4f}")
        print("  Confusion matrix (rows=true, cols=predicted):")
        print(
            format_confusion_matrix_text(
                c["confusion_matrix"],
                c["confusion_matrix_row_labels"],
            )
        )
        print()
