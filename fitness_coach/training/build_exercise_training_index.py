...
from fitness_coach.core.exercise_label_utils import (
...
def main() -> int:
    ...
    p.add_argument(
        "--fine-labels",
        default="./data/annotations/labels/fine_grained_labels.json",
        help="QEVD-style fine_grained_labels.json (optional; capstone uses Riccio NPZs by default).",
    )
    p.add_argument(
        "--feedbacks",
        default="./data/annotations/labels/feedbacks_short_clips.json",
    )
    p.add_argument(
        "--dataset-root",
        default="./data",
        help="Root containing videos/short_clips for require-file checks.",
    )
    ...
    if not rows:
        print(
            ...
            "  • Or restore/copy real .mp4 files into <dataset-root>/videos/short_clips/\n"
