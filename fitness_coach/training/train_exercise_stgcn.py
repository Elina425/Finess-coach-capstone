...
**Why keypoints, not joint angles?** ST-GCN treats the pose as a **graph**: nodes are the 17 COCO
joints, edges follow the body topology, and each node carries **2D coordinates** (x, y) per frame.
Graph convolutions **aggregate along bones**; that matches xy per joint. **Angles** are **scalar
features per limb** (or derived from triplets of joints)—they are not a (V, 2) signal on the same
graph, so this architecture would need a different graph or input tensor to consume angle streams
directly (the BiLSTM path uses `(T, 8)` angles instead).
...
from fitness_coach.models.exercise_stgcn_model import ExerciseSTGCN


def compute_inverse_frequency_class_weights(train_ds, num_classes: int) -> torch.Tensor:
    """Mean-normalized inverse-frequency weights from training window labels (same scheme as BiLSTM)."""
    samples = getattr(train_ds, "samples", None)
    if not samples:
        raise ValueError("empty train dataset or missing .samples")
...
def evaluate(
    ...
    *,
    ...
    if detailed:
        ...
        return acc, rmse, metrics, y_true_arr, y_prob_arr
    return acc, rmse
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        action="store_true",
        help="Inverse-frequency weights in cross-entropy (helps rare classes).",
    )
    args = ap.parse_args()
    ...
    if getattr(args, "balanced_class_weights", False):
        ce_w = compute_inverse_frequency_class_weights(train_ds, len(classes))
        print(
