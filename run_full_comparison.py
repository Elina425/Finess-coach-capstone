...
def aggregate_benchmark_over_videos(
    ...
    from model_comparison import ComparisonMetrics, ModelComparison
...
def discover_mp4_paths(dataset_root: str) -> List[Path]:
    """All .mp4 files under ``dataset_root/videos`` (recursive)."""
    root = Path(dataset_root) / "videos"
    if not root.is_dir():
        return []
...
def run_full_pipeline(
    dataset_root: str = "./qevd-fit-coach-data",
    num_videos: int = 3,
    *,
    seed: int = 42,
    randomize: bool = True,
    ...
    speed/accuracy metrics across the same clips → ``comparison_summary.png`` + JSON.
    ...
    print("\n[1/4] Loading Dataset...")
    all_paths = discover_mp4_paths(dataset_root)
    print(f"✓ Found {len(all_paths)} video files under {dataset_root}/videos")

    sample_paths = select_sample_video_paths(
        all_paths, num_videos, seed=seed, randomize=randomize
    )
    if not sample_paths:
        print("✗ No videos to process — check dataset_root and videos/")
        return {
            "visualizations": [],
            "summary_file": None,
            "metrics_json": None,
    ...
    print("\n[2/4] Pose models (MediaPipe, YOLO, RTMPose-X, ViTPose, DETRPose, …) load per visualization…")
    ...
    if video_paths_for_benchmark:
        metrics_by_key = aggregate_benchmark_over_videos(
            video_paths_for_benchmark,
        ...
        if metrics_by_key:
            ...
            generate_comparison_summary_png(
                metrics_by_key,
                best,
                video_count=len(video_paths_for_benchmark),
